...
EXPECTED_ROWS = {"train": 1649, "val": 602}
...
def validate_corpus(a):
    ...
    for name in ("train", "val"):
        ...
        try:
            df = pl.read_parquet(f, columns=["pair_id", "cap_a", "cap_b", "y"])
        except Exception as e:
            raise ValueError(f"{name}.parquet unreadable or missing required columns: {e}") from e
        if (
            df.height != EXPECTED_ROWS[name]
            or any(df.null_count().row(0))
            or df["pair_id"].n_unique() != df.height
        ):
            raise ValueError(
                f"{name}.parquet: expected exactly {EXPECTED_ROWS[name]} rows, no nulls, unique pair_id"
            )
        ydt = df.schema["y"]
        if (
            not ydt.is_integer()
        ...
        caps[name] = set(df["cap_a"].to_list()) | set(df["cap_b"].to_list())
        if any(not isinstance(c, str) or not c.strip() for c in caps[name]):
            raise ValueError(f"{name}.parquet: cap_a/cap_b must be nonempty nonnull text strings")
    ...
    if caps["train"] & caps["val"]:
        raise ValueError("train and val caption content sets intersect")
