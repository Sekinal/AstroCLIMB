...
`run_selected.py` is the only portability adaptation: it relocates the base-model loader without rewriting historical checkpoint metadata, preserves the source hashes required by IXI/CXC, and applies the historical Pillow PNG text-chunk allowance of 16 MiB (64 MiB aggregate unchanged). The wrapper keeps two identities apart: the logical model string recorded in checkpoint metadata and passed as `--model`, and the physical pinned snapshot directory actually used at load time. New training without a parent records the supplied physical snapshot path. Training with `--init-checkpoint` or `--resume` and no explicit `--model` reuses the parent metadata `args.model` string verbatim as the CLI value so the native equality check passes, while the loader still uses the verified pinned snapshot; parent files are only read, never rewritten. The parent value must be the configured model ID or a path whose basename is the pinned revision, and an explicit `--model` must satisfy the same check and equal the parent value when a parent is given; a contradiction is reported instead of being silently overridden. When the logical value differs from the physical snapshot the wrapper prints a JSON line with the canonical configured model ID as `logical_model` and the pinned revision; when the CLI value is an archival absolute path the receipt adds `archival_path_identity: true` and never prints that absolute path. Exact `--model`, `--init-checkpoint` and `--resume` spellings (including `=` syntax) remain supported, while abbreviated prefixes such as `--init`, `--resum` or `--mode` are rejected before the native parser runs; unrelated flags such as `--max-pixels` are unaffected. Exact resume requires keeping that path stable because the native trainer validates its saved model argument. The original eight files remain unchanged. The portability module and CPU tests were necessarily authored during curation after prior preferred-provider failures; they are not attributed to DeepSeek.
...
Historical source files were byte-hash checked; selected checkpoint manifests were checked against actual source-workspace files. CPU tests exercise portable model relocation, preservation of archival config and hook restoration, rejection of a wrong snapshot before loading imports, parent training relocation (logical identity passthrough with physical loading and untouched metadata), resume passthrough, rejection of an incompatible parent or a contradicting explicit model before the trainer starts, and unchanged physical recording for parent-free training. No clean-repository GPU fit, export or score reproduction has been performed.
