...
We hash source, data, model, and prediction artifacts and require fresh replay before accepting exports. Inference uses batch size one to match training. For the public2k-to-gold837 model, batch two changed 11 of 794 CXI predictions, with a maximum probability difference of 0.380. Source, prompts, and configurations are public; adapter/head weights are released separately.\ifdefined\CameraReady\footnote{\url{https://github.com/Sekinal/AstroCLIMB}; model downloads are linked there.}\else\footnote{Repository and model links omitted for anonymous review.}\fi Historical replay probes remain private because they contain training examples. Full retraining from the release has not been independently verified.
...
We use vLLM \citep{vllm} for rollout generation but retain native inference for evaluation because predictions vary across backends. Batch shape and BF16 autocast are matched across scoring and reload checks.

\paragraph{Sources of exploratory ideas.}
Visual-RFT and VLM-R1 \citep{visualrft,vlmr1} motivated visual rewards; DeepEyes \citep{deepeyes} motivated fixed-view trials. RL coverage and reasoning work \citep{rlvrcoverage,rlvrreasoning} informed the distinction between sampled and single-answer accuracy.
